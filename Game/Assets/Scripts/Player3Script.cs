...
public class Player3Script : MonoBehaviour
{
    ...
    {
        return x >= 0 && x < 7 && y >= 0 && y < 7 && grid.gridArray[x,y] != 3;
    }
    ...
    public List<Node> BFS()
    {
        ...
        mapa[startX,startY].distance=0;
        pq.Add(mapa[startX, startY]);
        ...
        while (pq.Count > 0)
        {
            ...
            for (int i = 0; i < dx.Length; i++)
            {
                ...
                if (IsValid(nx, ny))
                {
                    ...
                    if (newDistance < mapa[nx, ny].distance)
                    {
                        //Node newNode = new Node(nx, ny, newDistance);
                        //newNode.prevNode = node; // dodaj prethodnika
                        //Debug.Log(nx + " " + ny);
                        mapa[nx, ny].distance = newDistance;
                        mapa[nx, ny].x = nx;
                        mapa[nx, ny].y = ny;
                        mapa[nx, ny].prevNode = node;
                        pq.Add(mapa[nx,ny]);
                    }
                }
            }
